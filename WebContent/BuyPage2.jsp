...
<div class="container" id="container">
  <div class="form-container sign-up-container">
    <form method="post" action="BuyBidSho.sho" onsubmit="return validateForm()">      
	<h1>구매 입찰</h1>
	<br>
      <div style="border-radius: 10px; overflow: hidden;" class="social-container">
        ...
        <b class="text-sm"><%=request.getParameter("size") %></b>
                ...
                <div class="mb-4">
                    <label for="price" class="block text-sm font-medium text-gray-700">구매 희망가</label>
                    <input type="text" id="price" name="price" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" 
                    	placeholder="원하는 구매 희망가를 적어주세요.">
                </div>               
                ...
                <div> 
                 <button>구매 희망가 설정</button>
                </div>
    </form>  
  </div>
  
  <div class="form-container sign-in-container">
    <form action="BuySho.sho" method="post">
    <h1>즉시 구매 하기</h1>
    <br>
	 <div style="border-radius: 10px; overflow: hidden;" class="social-container">
        <img src="<%=request.getParameter("shoeimage")%>"style="width: 100%; height: auto;">
      ...
      <div class="mb-4">
		<label for="instantPrice"
		class="block text-sm font-medium text-gray-700">즉시 구매 가격</label>
		<%if(!request.getParameter("price").equals("0")){ %> 
		<input type="text" id="instantPrice"
		class="mt-1 block w-full border-gray-300 rounded-md shadow-sm"
		placeholder=<%=request.getParameter("price") + "원"%> disabled>	
		</div>
   		<input type="hidden" name="checkprice" value="<%=request.getParameter("checkprice")%>">
    	<input type="hidden" name="size" value="<%=request.getParameter("size")%>">
    	<input type="hidden" name="price" value="<%=request.getParameter("price")%>">
    	<input type="hidden" name="model" value="<%=request.getParameter("model")%>">
    	<input type="hidden" name="shoeimage" value="<%=request.getParameter("shoeimage")%>">
    	<div>
    	<button type="submit">즉시 구매</button>
    <%}else{ %>
	    <input type="text" id="instantPrice"
		class="mt-1 block w-full border-gray-300 rounded-md shadow-sm"
  ...
  <div class="overlay-container">
    <div class="overlay">
      <div class="overlay-panel overlay-left">
        <h1>즉시구매는<br>1~2일 안에 즉시 배송!</h1>
        <p>신발이 적정가격이라 생각 하신다면 <br>즉시! 배송 받는 즉시구매를 추천드려요!</p>
        <button class="ghost" id="signIn">즉시 구매 바로가기</button>
      </div>
      <div class="overlay-panel overlay-right">
        <h1>희망 구매가로 <br>구매해보세요!</h1>
        <p>내가 사고싶은 가격, 화혜장의 <br>경매 시스템을 이용해보세요.</p>
        <button class="ghost" id="signUp">희망가 구매 바로가기</button>
      </div>
    </div>
  </div>
</div>
